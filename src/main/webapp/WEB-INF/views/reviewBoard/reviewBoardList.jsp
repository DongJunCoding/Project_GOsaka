...
<%
ArrayList<ReviewBoardTO> reviewBoardList = (ArrayList) request.getAttribute("reviewBoardList");
PageListTO pageTO = (PageListTO) request.getAttribute("pageTO");
ArrayList<ReviewBoardTO> reviewPageList = pageTO.getReviewBoardPageList();
...
totalsize = reviewBoardList.size();
...
int hit = 0;
int rvgap = 0;
if (reviewPageList.size() != 0) {
	for (ReviewBoardTO to : reviewPageList) {
		String reviewId = to.getReviewId();
		category = to.getCategory();
		subject = to.getRvsubject();
		writer = to.getRvwriter();
		date = to.getRvdate();
		ip = to.getIp();
		hit = to.getRvhit();
		rvgap = to.getRvgap();
		...
		sbHtml.append("<td >" + reviewId + "</td>");
		sbHtml.append("<td><strong style='color:coral;'>"+ category +"</strong>");
		sbHtml.append("<a href='/reviewBoardView.do?reviewId="+reviewId+"&cpage="+cpage+"'>&nbsp;&nbsp;"+ subject + "</a>");
		if (rvgap == 0) {
...
<html class="no-js" lang="en">
    ...
    <div class="bcontainer">
      ...
      <div class="table-responsive">
        ...
        %>	
       	<button type="button"style="float:right;"><a href="/reviewBoardWrite.do?cpage=<%=cpage%>">글쓰기</a></button>
      	<%
        	}
      	%>  	     
      </div>
		...
		<div>
			...
			if (searchWord == null) {
				if (startBlock == 1) {
					...
					out.println("<span><a href='/reviewBoardList.do?cpage=" + (startBlock - blockPerPage) + "'>&lt;&lt;</a></span>");
				}
				...
				if (cpage == 1) {
					...
					out.println("<span><a href='/reviewBoardList.do?cpage=" + (cpage - 1) + "'>&lt;</a></span>");
				}
				out.println("&nbsp;&nbsp;");
				...
				out.println("<span><a href='/reviewBoardList.do?cpage=" + i + "'>" + i + "</a></span>");
					}
				}
				...
				if (startBlock == 1) {
					...
					out.println("<span><a href='/reviewBoardList.do?cpage=" + (startBlock - blockPerPage) + "&searchWord="+searchWord+"'>&lt;&lt;</a></span>");
				}
				...
				if (cpage == 1) {
					...
					out.println("<span><a href='/reviewBoardList.do?cpage=" + (cpage - 1) + "&searchWord="+searchWord+"'>&lt;</a></span>");
				}
				out.println("&nbsp;&nbsp;");
				for (int i = startBlock; i <= endBlock; i++) {
					if (i == cpage) {
				...
				// 페이지 번호들 나열해주고 해당 페이지의 데이터(게시글)들을 보여준다
				out.println("<span><a href='/reviewBoardList.do?cpage=" + i + "&searchWord="+searchWord+"'>" + i + "</a></span>");
					}
				}
