...
<%
ArrayList<QnaBoardTO> qnaBoardList = (ArrayList) request.getAttribute("qnaBoardList");
PageListTO pageTO = (PageListTO) request.getAttribute("pageTO");
ArrayList<QnaBoardTO> qnaPageList = pageTO.getQnaBoardPageList();
...
int hit = 0;
int tgap = 0;
if (qnaPageList.size() != 0) {
	for (QnaBoardTO to : qnaPageList) {
		String qnaId = to.getQnaId();
		subject = to.getQsubject();
		writer = to.getQwriter();
		date = to.getQdate();
		ip = to.getIp();
		hit = to.getQhit();
		tgap = to.getQgap();
		...
		sbHtml.append("<td >" + qnaId + "</td>");
		sbHtml.append("<td><a href='/qnaBoardView.do?qnaId="+qnaId+"&cpage="+cpage+"'>" + subject + "</a>");
		if (tgap == 0) {
...
<html class="no-js" lang="en">
    ...
    <div class="bcontainer">
      ...
      <span data-filter="identity" style="float: right; display: flex; align-items: center; padding-bottom:20px;">
	        <form action="/qnaBoardList.do" method="post" name="ofrm" style="display: flex; align-items: center;">
		        <select name="searchKey" style="height: 25px; font-size:15px;">
					<option value="qsubject">제목</option>
					<option value="qcontent">내용</option>
					<option value="qwriter">이름</option>
				</select>
      ...
      <div class="table-responsive">
        ...
        %>
   		<a><button type="button"style="float:right;" id="write-btn">글쓰기</button></a>
        <%
        	} else if(role != null || !role.equals("BLACK")) {
        %>	
       	<button type="button"style="float:right;"><a href="/qnaBoardWrite.do?cpage=<%=cpage%>">글쓰기</a></button>
      	<%
        	}
      	%>  	
		...
		<div>
			...
			if (searchWord == null) {
				if (startBlock == 1) {
					...
					out.println("<span><a href='/qnaBoardList.do?cpage=" + (startBlock - blockPerPage) + "'>&lt;&lt;</a></span>");
				}
				...
				if (cpage == 1) {
					...
					out.println("<span><a href='/qnaBoardList.do?cpage=" + (cpage - 1) + "'>&lt;</a></span>");
				}
				out.println("&nbsp;&nbsp;");
				...
				out.println("<span><a href='/qnaBoardList.do?cpage=" + i + "'>" + i + "</a></span>");
					}
				}
				...
				if (startBlock == 1) {
					...
					out.println("<span><a href='/qnaBoardList.do?cpage=" + (startBlock - blockPerPage) + "&searchWord="+searchWord+"'>&lt;&lt;</a></span>");
				}
				...
				if (cpage == 1) {
					...
					out.println("<span><a href='/qnaBoardList.do?cpage=" + (cpage - 1) + "&searchWord="+searchWord+"'>&lt;</a></span>");
				}
				out.println("&nbsp;&nbsp;");
				for (int i = startBlock; i <= endBlock; i++) {
					if (i == cpage) {
				...
				// 페이지 번호들 나열해주고 해당 페이지의 데이터(게시글)들을 보여준다
				out.println("<span><a href='/qnaBoardList.do?cpage=" + i + "&searchWord="+searchWord+"'>" + i + "</a></span>");
					}
				}
