...
<%
QnaBoardTO to = (QnaBoardTO) request.getAttribute("qnaBoardView");
ArrayList<CommentTO> qna_commentList = (ArrayList)request.getAttribute("qna_commentList");
...
String[] images = to.getImage().split("▒");

String qsubject = to.getQsubject();
String qwriter = to.getQwriter();
String qdate = to.getQdate();
int qhit = to.getQhit();
String qcontent = to.getQcontent().replaceAll("\n", "<br />");

StringBuffer commentHtml = new StringBuffer();
for (CommentTO commentTO : qna_commentList) {
	   ...
	   if((role != null && role.equals("ADMIN")) || commentTO.getCwriter().equals(nickSession)) {   
	      ...
	      commentHtml.append("<form action='/qna_CommentDeleteOk.do' method='post' name='cdfrm' style='display: inline;'>");
	      commentHtml.append("<input type='hidden' name='qnaId' value='" + qnaId+ "'/>");
	   ...
	   commentHtml.append("<form action='/qna_CommentModifyOk.do' method='post' name='editCommentForm'>");
	   commentHtml.append("<input type='hidden' name='qnaId' value='" + qnaId + "'/>");
	   commentHtml.append("<input type='hidden' name='cmId' value='" + cmId + "' />");
...
<html class="no-js" lang="en">
	...
	<!-- Preloader -->
        ...
        <section id="qnaBoard" class="home bg-black fix">
                        ...
                        <div class="main_home text-center">
                            <div class="col-md-12">
                                <div class="hello">
                                    <div class="slid_item">
                                        <div class="home_text ">
                                            <h1 class="text-yellow">Q&A Board </h1>                                                                              
		...
		<!--게시판-->
			...
			<div class="board_view">		
				...
				<tr style="height:70px;">  
					...
					<td width="60%" style="font-size:20px;"><%=qsubject %></td>
					<th width="10%" style="font-size:20px;">등록일</th>
					<td width="20%" style="font-size:20px;"><%=qdate %></td>
				</tr>
				<tr style="height:70px;">
					<th style="font-size:20px;">글쓴이</th>
					<td style="font-size:20px;"><%=qwriter %></td>
					<th style="font-size:20px;">조회</th>
					<td style="font-size:20px;"><%=qhit %></td>
				</tr>				
				<tr>
				...
				<form action="/qna_CommentOk.do" method="post" name="cfrm">
			...
			<div class="btn_area">
				<div class="align_left">			
					<input type="button" value="목록" class="btn_list btn_txt02" style="cursor: pointer;" onclick="location.href='/qnaBoardList.do?cpage=<%=cpage%>'" />
				</div>
				<div class="align_right">
				<%
					if((role != null && role.equals("ADMIN")) || qwriter.equals(nickSession)) {
				%>
					<input type="button" value="수정" class="btn_list btn_txt02" style="cursor: pointer;" onclick="location.href='/qnaBoardModify.do?qnaId=<%=qnaId %>&cpage=<%=cpage %>'" />
	  			    <input type="button" value="삭제" class="btn_list btn_txt02" style="cursor: pointer;" onclick="location.href='/qnaBoardViewDelete_ok.do?qnaId=<%=qnaId %>'" /> 
				<%
					} 
				%>	
