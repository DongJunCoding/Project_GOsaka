...
<%
TipBoardTO tipBoardTO = (TipBoardTO) request.getAttribute("tipBoardView");
ArrayList<CommentTO> tip_commentList = (ArrayList)request.getAttribute("tip_commentList");
...
String tipId = tipBoardTO.getTipId();
String cmId = null;

String[] images = tipBoardTO.getImage().split("▒");

String tsubject = tipBoardTO.getTsubject();
String twriter = tipBoardTO.getTwriter();
String tdate = tipBoardTO.getTdate();
int thit = tipBoardTO.getThit();
String tcontent = tipBoardTO.getTcontent().replaceAll("\n", "<br />");


StringBuffer commentHtml = new StringBuffer();
for (CommentTO commentTO : tip_commentList) {	
	...
	if((role != null && role.equals("ADMIN")) || commentTO.getCwriter().equals(nickSession)) { 
	   ...
	   commentHtml.append("<form action='/tip_CommentDeleteOk.do' method='post' name='cdfrm' style='display: inline;'>");
	   commentHtml.append("<input type='hidden' name='tipId' value='" + tipId+ "'/>");
	...
	commentHtml.append("<form action='/tip_CommentModifyOk.do' method='post' name='editCommentForm'>");
	commentHtml.append("<input type='hidden' name='tipId' value='" + tipId + "'/>");
	commentHtml.append("<input type='hidden' name='cmId' value='" + cmId + "' />");
...
<html class="no-js" lang="en">
	...
	<body>
        ...
        <section id="tipBoard" class="home bg-black fix">
                        ...
                        <div class="main_home text-center">
                            <div class="col-md-12">
                                <div class="hello">
                                    <div class="slid_item">
                                        <div class="home_text ">
                                            <h1 class="text-yellow">Tip Board </h1>                                                                              
		...
		<!--게시판-->
			...
			<div class="board_view">		
				...
				<tr style="height:70px;">  
					...
					<td width="60%" style="font-size:20px;"><%=tsubject %></td>
					<th width="10%" style="font-size:20px;">등록일</th>
					<td width="20%" style="font-size:20px;"><%=tdate %></td>
				</tr>
				<tr style="height:70px;">
					<th style="font-size:20px;">글쓴이</th>
					<td style="font-size:20px;"><%=twriter %></td>
					<th style="font-size:20px;">조회</th>
					<td style="font-size:20px;"><%=thit %></td>
				</tr>				
				<tr>
				...
				<form action="/tip_CommentOk.do" method="post" name="cfrm">
			...
			<div class="btn_area">
				<div class="align_left">			
					<input type="button" value="목록" class="btn_list btn_txt02" style="cursor: pointer;" onclick="location.href='/tipBoardList.do?cpage=<%=cpage%>'" />
				</div>
				<div class="align_right">
				<%
					if((role != null && role.equals("ADMIN")) || twriter.equals(nickSession) ) {
				%>
					<input type="button" value="수정" class="btn_list btn_txt02" style="cursor: pointer;" onclick="location.href='/tipBoardModify.do?tipId=<%=tipId %>&cpage=<%=cpage %>'" />
	  			    <input type="button" value="삭제" class="btn_list btn_txt02" style="cursor: pointer;" onclick="location.href='/tipBoardViewDelete_ok.do?tipId=<%=tipId %>'" /> 
				<%
					}
				%>
				</div>
