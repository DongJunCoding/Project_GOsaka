...
<%
ArrayList<TipBoardTO> tipBoardList = (ArrayList) request.getAttribute("tipBoardList");
PageListTO pageTO = (PageListTO) request.getAttribute("pageTO");
ArrayList<TipBoardTO> tipPageList = pageTO.getTipBoardPageList();
...
int hit = 0;
int tgap = 0;
if (tipPageList.size() != 0) {
	for (TipBoardTO to : tipPageList) {
		String tipId = to.getTipId();
		subject = to.getTsubject();
		writer = to.getTwriter();
		date = to.getTdate();
		ip = to.getIp();
		hit = to.getThit();
		tgap = to.getTgap();
		...
		sbHtml.append("<td >" + tipId + "</td>");
		sbHtml.append("<td><a href='/tipBoardView.do?tipId="+tipId+"&cpage="+cpage+"'>" + subject + "</a>");
		if (tgap == 0) {
...
<html class="no-js" lang="en">
    ...
    <div class="bcontainer">
      ...
      <span data-filter="identity" style="float: right; display: flex; align-items: center; padding-bottom:20px;">
	        <form action="/tipBoardList.do" method="post" name="ofrm" style="display: flex; align-items: center;">
		        <select name="searchKey" style="height: 25px; font-size:15px;">
					<option value="tsubject">제목</option>
					<option value="tcontent">내용</option>
					<option value="twriter">이름</option>
				</select>
      ...
      <div class="table-responsive">
        ...
        %>
   		<button type="button"style="float:right;"><a id="write-btn">글쓰기</a></button>
        <%
        	} else if(role != null || !role.equals("BLACK")) {
        %>	
       	<button type="button"style="float:right;"><a href="/tipBoardWrite.do?cpage=<%=cpage%>">글쓰기</a></button>
      	<%
        	}
      	%>  	
		...
		<div>
			...
			if (searchWord == null) {
				if (startBlock == 1) {
					...
					out.println("<span><a href='/tipBoardList.do?cpage=" + (startBlock - blockPerPage) + "'>&lt;&lt;</a></span>");
				}
				...
				if (cpage == 1) {
					...
					out.println("<span><a href='/tipBoardList.do?cpage=" + (cpage - 1) + "'>&lt;</a></span>");
				}
				out.println("&nbsp;&nbsp;");
				...
				out.println("<span><a href='/tipBoardList.do?cpage=" + i + "'>" + i + "</a></span>");
					}
				}
				...
				if (startBlock == 1) {
					...
					out.println("<span><a href='/tipBoardList.do?cpage=" + (startBlock - blockPerPage) + "&searchWord="+searchWord+"'>&lt;&lt;</a></span>");
				}
				...
				if (cpage == 1) {
					...
					out.println("<span><a href='/tipBoardList.do?cpage=" + (cpage - 1) + "&searchWord="+searchWord+"'>&lt;</a></span>");
				}
				out.println("&nbsp;&nbsp;");
				for (int i = startBlock; i <= endBlock; i++) {
					if (i == cpage) {
				...
				// 페이지 번호들 나열해주고 해당 페이지의 데이터(게시글)들을 보여준다
				out.println("<span><a href='/tipBoardList.do?cpage=" + i + "&searchWord="+searchWord+"'>" + i + "</a></span>");
					}
				}
