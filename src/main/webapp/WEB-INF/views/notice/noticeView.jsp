...
<%
NoticeTO noticeTO = (NoticeTO) request.getAttribute("noticeView");
...
String noticeId = noticeTO.getNoticeId();

String[] images = noticeTO.getImage().split("▒");

String nsubject = noticeTO.getNsubject();
String nwriter = noticeTO.getNwriter();
String ndate = noticeTO.getNdate();
int nhit = noticeTO.getNhit();
String ncontent = noticeTO.getNcontent().replaceAll("\n", "<br />");
		...
		<!--게시판-->
			...
			<div class="board_view">		
				...
				<tr style="height:70px;">  
					...
					<td width="60%" style="font-size:20px;"><%=nsubject %></td>
					<th width="10%" style="font-size:20px;">등록일</th>
					<td width="20%" style="font-size:20px;"><%=ndate %></td>
				</tr>
				<tr style="height:70px;">
					<th style="font-size:20px;">글쓴이</th>
					<td style="font-size:20px;"><%=nwriter %></td>
					<th style="font-size:20px;">조회</th>
					<td style="font-size:20px;"><%=nhit %></td>
				</tr>				
				<tr>
			...
			<div class="btn_area">
				...
				<%
					if(role != null && role.equals("ADMIN")) {
				%>
					<input type="button" value="수정" class="btn_list btn_txt02" style="cursor: pointer;" onclick="location.href='/noticeModify.do?noticeId=<%=noticeId %>&cpage=<%=cpage %>'" />
	  			    <input type="button" value="삭제" class="btn_list btn_txt02" style="cursor: pointer;" onclick="location.href='/noticeViewDelete_ok.do?noticeId=<%=noticeId %>'" /> 
				<%
					}
				%>
				</div>
