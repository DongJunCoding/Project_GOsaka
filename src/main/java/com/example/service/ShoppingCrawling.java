...
@Service("ShoppingCrawling")
public class ShoppingCrawling implements Sservice {

	@Autowired
	private ShopAndFoodMapperInter mapper;

	@Override
	public int getShoppingInfoFromWEB() throws Exception {
		int res = 0;

		// 크롤링 할 사이트 주소
		String baseUrl = "https://osaka-info.kr/shop-roadshop/?q=YToxOntzOjEyOiJrZXl3b3JkX3R5cGUiO3M6MzoiYWxsIjt9&page=";
		int lastPage = 3;
				...
				for (int i = 0; i < imgWrappers.size(); i++) {
					...
					if (!mapElements.isEmpty()) {
						...
						if (latitude != null && longitude != null) {
							ShoppingTO rsTO = new ShoppingTO();

							rsTO.setImageUrl(imageUrl);
							rsTO.setStitle(title);
							rsTO.setScontent(fullContent);
							rsTO.setLatitude(latitude);
							rsTO.setLongitude(longitude);
							res += mapper.shopCrawling(rsTO);
